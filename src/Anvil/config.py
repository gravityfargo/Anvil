from Anvil import AnvilData as ad
from Anvil import filemanager as fm
from Anvil.utilities import YamlManager
import os


def initialize():
    fm.check_dir(ad.options.temp_dir, create=True)

    if fm.check_file(ad.options.config_path, create=True):
        config = YamlManager.get_all(ad.options.config_path)

    YamlManager.create_or_update_item("options", config["options"])

    # ad.options = config["options"]
    # ad.debug = config["debug"]
    # ad.projects = config["projects"]
    # ad.s_project = config["s_project"]
    # ad.s_group = config["s_group"]
    # ad.s_host = config["s_host"]
